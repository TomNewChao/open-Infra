            ...
            for obj_data in list_data:
                try:
                    ...
                    with HWCloudObs(settings.OBS_AK, settings.OBS_SK, settings.OBS_BASE_URL) as hw_clouds_obs:
                        ...
                        default_policy = hw_clouds_obs.get_obs_default_policy(settings.OBS_DOMAIN_ID, user_id, path,
                                                                              username,
        ...
        with ScanToolsLock.obs_interact_lock:
            ...
            for obj_data in list_data:
                try:
                    ...
                    with HWCloudObs(settings.OBS_AK, settings.OBS_SK, settings.OBS_BASE_URL) as hw_clouds_obs:
                        md5sum_not_con, sen_file, lack_file, sen_dict = ObsTools.check_bucket_info_and_mdsum(
                            hw_clouds_obs.obs_client,
                            md5sum_dict,
                            prefix=obs_prefix,
                        ...
                        if len(md5sum_not_con) or len(sen_file) or len(lack_file) or len(sen_dict):
                            ...
                            if len(hw_clouds_obs_interact):
                                ...
                                HWColudObsInteract.objects.filter(community=community, username=username).update(
                                    is_delete=True)
                    ...
                    logger.error(
                        "[ObsInteractMgr] check_upload_process e:{},trace_back:{}".format(e, traceback.format_exc()))
