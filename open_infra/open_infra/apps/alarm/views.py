...
class AlarmView(GenericViewSet):
    permission_classes = (permissions.IsAuthenticated,)
    authentication_classes = (authentication.JWTAuthentication,)

    def list(self, request):
        params_dict = list_param_check_and_trans(request.GET.dict(), order_by="alarm_refresh_time")
        alarm_mgr = AlarmMgr()
...
class AlarmNotifyView(GenericViewSet):
    permission_classes = (permissions.IsAuthenticated,)
    authentication_classes = (authentication.JWTAuthentication,)
    ...
    def retrieve(self, request, pk=None):
        if pk is None:
            return assemble_api_result(ErrCode.STATUS_PARAMETER_ERROR)
        try:
            alarm_notify = AlarmNotify.objects.get(id=pk)
            dict_data = alarm_notify.to_dict()
            alarm_notify_strategy_list = AlarmNotifyStrategy.objects.filter(alarm_notify__id=pk)
            dict_data["alarm_name"] = [alarm_notify_strategy.alarm_name for alarm_notify_strategy in
                                       alarm_notify_strategy_list]
    ...
    def update(self, request, pk=None):
        ...
        if len(desc) > 255:
            return assemble_api_result(ErrCode.STATUS_PARAMETER_ERROR)
        try:
            alarm_notify_obj = AlarmNotify.objects.get(id=pk)
            with transaction.atomic():
                AlarmNotify.objects.filter(id=pk).update(
                    email=email, phone_number=phone_number, desc=desc,
                )
                AlarmNotifyStrategy.objects.filter(alarm_notify_id=pk).delete()
                for alarm_name in alarm_name_list:
                    AlarmNotifyStrategy.objects.create(alarm_name=alarm_name, alarm_keywords=keywords,
                                                       alarm_notify=alarm_notify_obj)
        except AlarmNotify.DoesNotExist:
